...
def extract_chords_legacy(pm: pretty_midi.PrettyMIDI) -> List[Tuple[float, float, str]]:
  midi_chord = MIDIChord(pm)
  chords = midi_chord.extract()
  chords = [(chord[0], chord[1], chord[2].replace("N:N", "N").replace("dom7", "7")) for chord in chords]
  return chords
